...
model= xgb.fit(X_train,y_train,sample_weight=classes_weights)
...
xgb_model = xgb_grid_model.fit(X_train, y_train,sample_weight=classes_weights)
y_pred_train = xgb.predict()
...
score = cross_val_score(xgb_model,X,y,,scoring='accuracy')
...
steps = [('over', SMOTE()), ('model', XGBClassifier(learning_rate = xgb_grid.best_params_.get('learning_rate'),
                               max_depth = xgb_grid.best_params_.get('max_depth'),
                               gamma = xgb_grid.best_params_.get('gamma')))]
...
score= cross_val_score(model,X,y,scoring='roc_auc',cv=kfold,n_jobs=-1)
...
def BalancedSampleWeights(y_train,class_weight_coef):
    ...
    [classes[1]] = class_weight_dict[classes[1]] * class_weight_coef

    sample_weights = [class_weight_dict[i] for i in y_train]
    return sample_weights
#Usage
weight=BalancedSampleWeights(y_test,class_weight_coef)
